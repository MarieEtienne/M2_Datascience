...
Grossièrement dit, c'est la,probabilité d'observer Fobs ou des valeurs encore plus extrèmes sous HO.

--

On va rejeter H0 lorsque la probabilité critique est faible (typiquement inférieure à 5%). La valeur de la statistique de test observée est peu compatible avec l'hypothèse H0. On ne croit pas à H0.
...
Si on s'intéresse au $\mbox{j}^{eme}$ terme du vecteur $\theta$  ( $\theta_j$ ):
...
---
# Bilan su rle modèle d'analyse de la variance à 1 facteur
Le modèle 

- permet d'étudier le lein entre une variable quantitative (la fréquence cardiaque) et un facteur (le Sexe).
-  s'écrit sous forme
...
- doit être précisé par une contrainte. R par défaut choisit la contrainte $\alpha_1=0$, ce qui place le groupe 1 comme groupe de référence.
- s'écrit sous la forme générqiue $Y = X\theta +E$

Il est possible de  
- comparer des modèles ( pour tester l'effet du facteur),
- tester si un paramètre vaut une valeur donnée (par défaut R teste systématiquement si un paramètre vaut 0)
