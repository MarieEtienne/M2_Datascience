...
Si on s'intéresse au j$^{eme}$ du vecteur $\theta$, $\theta_j$, sous l'hypothèse $H_0:\left\lbrace \theta_j = 0\right\rbrace,$ 
...
$$H_0 = \left \lbrace \theta_j = 0 \right\rbrace$$
...
```{r test_alpha1, eavl =TRUE, echo = FALSE}
M1 <- lm(freqC~Sexe, data = freqdata)
summary(M1)
```
...
- $\hat{\sigma}=$ `r summary(M1)$sigma`.
- $\sqrt{c_j}\hat{\sigma}=$ `r summary(M1)$coefficients[1,2]`,
- $t_{j,obs}=$ `r summary(M1)$coefficients[1,3]`,
- $p-value=$ `r summary(M1)$coefficients[1,4]`,
